    ...
    def process_task(self, task_id: int):
        ...
        try:
            ...
            if not results:
                log_data = {
                    "type": "warning", 
                    "message": "⚠️ هیچ داده‌ای برای سوال شما یافت نشد."
                }
                self.send_log_to_user(user_id, task_id, log_data)
            ...
            if len(results) > 0:
                ...
                log_data = {
                    "type": "log", 
                    "message": f"📊 نمونه نتایج:\n{sample_results}"
                }
                self.send_log_to_user(user_id, task_id, log_data)

            # Step 4: Generate final response using test_001 response generator
            log_data = {"type": "log", "message": "🤖 در حال تولید پاسخ نهایی..."}
            self.send_log_to_user(user_id, task_id, log_data)
